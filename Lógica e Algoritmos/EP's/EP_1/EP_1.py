letras_tipo_zumbi = ["z", "m", "b"]
preposicoes_txtB, verbos_txtB, verbos_prim_pessoa_txtB = 0, 0, 0
...
def eh_preposicao(palavra: str) -> bool:
    return palavra[0] in letras_tipo_zumbi and palavra[-1] not in letras_tipo_zumbi

def eh_verbo(palavra: str) -> bool:
    return len(palavra) == 7 and palavra[-1] not in letras_tipo_zumbi

def verbo_primeira_pessoa(palavra: str) -> bool:
    return palavra[0] not in letras_tipo_zumbi

def sort_list(txt):
    return sorted(txt, key=lambda palavra: [alfabeto.index(i) for i in palavra])

def digitos_distintos(num: int) -> bool:
    str_num = str(num)
    for digito in str_num:
        if str_num.count(str(digito)) == 2:
            return False
        
    return True

def eh_magico(palavra: str, num: int) -> bool:
    return num % 42 == 0 and digitos_distintos(palavra)

def get_numeros_magicos(texto: list) -> list:
    cont = 0
    numeros_magicos = []
    for palavra in texto:
        for i in range(len(palavra)):
            cont += alfabeto.index(palavra[i]) * 20 ** i
        
        if eh_magico(palavra, cont): numeros_magicos.append([palavra, cont])
...
for p in txtB:
    if eh_preposicao(p): preposicoes_txtB += 1
    
    if eh_verbo(p):
        verbos_txtB += 1

        if verbo_primeira_pessoa(p): verbos_prim_pessoa_txtB += 1


print(f"a) {preposicoes_txtB}")
print(f"b) {verbos_txtB}")
print(f"c) {verbos_prim_pessoa_txtB}")
print(f"d) {' '.join(sort_list(txtB))}")
print(f"e) {get_numeros_magicos(txtB)}")
