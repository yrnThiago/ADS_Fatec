...
def maior_ponta(nums: list) -> list:
  return [max(nums[0], nums[-1]) for _ in nums]
...
# date_fashion(5, 2) -> 0
# date_fashion(5, 5) -> 1
def date_fashion(eu: int, par: int) -> int:
...
def squirrel_play(temp: int, is_summer: bool) -> bool:
  max_temp = 90 if not is_summer else 100
  return 60 <= temp <= max_temp
...
def pego_correndo(speed: int, is_birthday: bool) -> int:
