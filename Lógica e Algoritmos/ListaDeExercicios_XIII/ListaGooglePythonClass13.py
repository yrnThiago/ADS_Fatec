...
def fim_igual(words: list[str]) -> int:
  return len([w for w in words if w and len(w) >= 2 and w[0] == w[-1]])
...
def x_antes(words: list[str]) -> list[str]:
  return sorted(words, key=lambda palavra: ["xabcdefghijklmnopqrstuvwyz".index(i) for i in palavra])
...
def sort_last(tuples: list[tuple]) -> tuple:
